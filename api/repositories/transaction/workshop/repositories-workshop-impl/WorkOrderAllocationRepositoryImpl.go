...
func (r *WorkOrderAllocationRepositoryImpl) GetAll(tx *gorm.DB, companyCode int, foremanId int, date time.Time, filterCondition []utils.FilterCondition, pages pagination.Pagination) ([]map[string]interface{}, int, int, *exceptions.BaseErrorResponse) {
	...
	// if err := tx.Delete(&transactionworkshopentities.WorkOrderAllocationGrid{}).Error; err != nil {
	...
	//profitCenterGR := "00002" //tx.Raw("SELECT dbo.getVariableValue('PROFIT_CENTER_GR')").Scan(&profitCenterGR)

	// Insert technician and shift code from atAssignTech
	...
	if err := tx.Model(&transactionworkshopentities.AssignTechnician{}).
		...
		Where("company_id = ? AND foreman_id = ? AND service_date = ?", companyCode, foremanId, date).
	...
	// Insert data into atWoAllocateGrid table
	for _, assignTech := range assignTechnicians {
		...
		if err := tx.Create(&workordergrid).Error; err != nil {
			return nil, 0, 0, &exceptions.BaseErrorResponse{
				StatusCode: http.StatusInternalServerError,
				Message:    "Failed to insert data into atWoAllocateGrid",
				Err:        err,
			}
		}
	}
	...
	for rows.Next() {
		var woResponse transactionworkshoppayloads.WorkOrderAllocationGridResponse
		if err := rows.Scan(
			&woResponse.ForemanId,
			&woResponse.ServiceDate,
			&woResponse.CompanyId,
		); err != nil {
			return nil, 0, 0, &exceptions.BaseErrorResponse{
				StatusCode: http.StatusInternalServerError,
...
func (r *WorkOrderAllocationRepositoryImpl) SaveAssignTechnician(tx *gorm.DB, date time.Time, techId int, id int, request transactionworkshoppayloads.WorkOrderAllocationAssignTechnicianRequest) (transactionworkshopentities.AssignTechnician, *exceptions.BaseErrorResponse) {
	...
	if err := tx.Where("company_id = ? AND service_datetime = ? AND technician_id = ? AND shift_code = ?",
