...
func (s *PriceListServiceImpl) GetPriceListByCodeId(CodeId string) (masteritempayloads.PriceListGetbyCode, *exceptions.BaseErrorResponse) {
	tx := s.DB.Begin()
	var err *exceptions.BaseErrorResponse
	...
	results, err := s.priceListRepo.GetPriceListByCodeId(tx, CodeId)
