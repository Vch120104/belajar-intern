...
func (s *WorkOrderAllocationServiceImpl) GetAll(companyCode int, foremanId int, date time.Time, filterCondition []utils.FilterCondition, pages pagination.Pagination) ([]map[string]interface{}, int, int, *exceptions.BaseErrorResponse) {
	ctx := context.Background()
	cacheKey := utils.GenerateCacheKeys("work_order_allocation", filterCondition, pages)
	...
	results, totalPages, totalRows, repoErr := s.WorkOrderAllocationRepository.GetAll(tx, companyCode, foremanId, date, filterCondition, pages)
	if repoErr != nil {
