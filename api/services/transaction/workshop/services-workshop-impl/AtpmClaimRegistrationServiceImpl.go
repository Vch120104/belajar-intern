...
func (s *AtpmClaimRegistrationServiceImpl) GetAllClaimHistory(filterCondition []utils.FilterCondition, pages pagination.Pagination) (pagination.Pagination, *exceptions.BaseErrorResponse) {
	tx := s.Db.Begin()
	var err *exceptions.BaseErrorResponse
	...
	results, repoErr := s.AtpmClaimRegistrationRepository.GetAllClaimHistory(tx, filterCondition, pages)
	if repoErr != nil {
		return results, repoErr
	}
...
func (s *AtpmClaimRegistrationServiceImpl) GetAllDetail(id int, filterCondition []utils.FilterCondition, pages pagination.Pagination) (pagination.Pagination, *exceptions.BaseErrorResponse) {
	tx := s.Db.Begin()
	var err *exceptions.BaseErrorResponse
	...
	results, repoErr := s.AtpmClaimRegistrationRepository.GetAllDetail(tx, id, filterCondition, pages)
	if repoErr != nil {
		return results, repoErr
	}
...
func (s *AtpmClaimRegistrationServiceImpl) GetDetailById(claimsysno int, id int, pages pagination.Pagination) (pagination.Pagination, *exceptions.BaseErrorResponse) {
	tx := s.Db.Begin()
	var err *exceptions.BaseErrorResponse
	...
	results, repoErr := s.AtpmClaimRegistrationRepository.GetDetailById(tx, claimsysno, id, pages)
	if repoErr != nil {
		return results, repoErr
	}
...
func (s *AtpmClaimRegistrationServiceImpl) AddDetail(claimsysno int, request transactionworkshoppayloads.AtpmClaimDetailRequest) (transactionworkshopentities.AtpmClaimVehicleDetail, *exceptions.BaseErrorResponse) {
	tx := s.Db.Begin()
	var err *exceptions.BaseErrorResponse
	...
	result, repoErr := s.AtpmClaimRegistrationRepository.AddDetail(tx, claimsysno, request)
	if repoErr != nil {
		return result, repoErr
	}
...
func (s *AtpmClaimRegistrationServiceImpl) DeleteDetail(detailId int, claimsysno int) (bool, *exceptions.BaseErrorResponse) {
	tx := s.Db.Begin()
	var err *exceptions.BaseErrorResponse
	...
	_, repoErr := s.AtpmClaimRegistrationRepository.DeleteDetail(tx, detailId, claimsysno)
	if repoErr != nil {
		return false, repoErr
	}
